...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dataset",
        type=Path,
        default=Path("techqa_documents.jsonl"),
        help="Path to techqa_documents_normalized.jsonl",
    )
    parser.add_argument(
        "--golden",
        type=Path,
        default=Path("golden_dataset.jsonl"),
...
def main(config, output_csv) -> None:
    ...
    results = indexer.run_benchmark(output_csv)
...
if __name__ == "__main__":
    config= Config(
        dataset_path=Path("techqa_documents.jsonl"),
        golden_path=Path("golden_dataset.jsonl"),
        qdrant_url="http://localhost:6333",
        pg_dsn="postgresql://postgres:postgres@localhost:5432/postgres",
        collection="helpdeskai_phase3",
        pg_table="helpdeskai_phase3",
        top_k=10,
        benchmark_limit=100,
        batch_size=128,
    )
    print(f"Using configuration: {config}")
    main(config, output_csv=Path("phase3_benchmark_results.csv"))
