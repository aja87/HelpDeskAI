...
from helpdeskai.rag.workflow import run_rag_core
...
def main() -> None:
    ...
    if not args.query:
        raise ValueError("--query is required when action=answer")
    payload = run_rag_core(config, query=args.query)
    logging.info("Query: %s", payload["query"])
    logging.info("Rewritten query: %s", payload["rewritten_query"])
    logging.info("Prompt: %s | Mode: %s", payload["prompt_variant"], payload["retrieval_mode"])
    for step in payload.get("steps", []):
        logging.info("Step %-16s %s", step.get("step", "?"), step)
    logging.info("Answer: %s", payload["answer"])
    for index, row in enumerate(payload["contexts"], start=1):
        logging.info("[%d] %s", index, row["chunk_id"])
        logging.info("    %s", row["compressed_text"])
